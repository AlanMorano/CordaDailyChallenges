    ...
    override fun verify(tx: LedgerTransaction) {
        ...
        requireThat {
            ...
            is Commands.Register -> {
                ...
                val regOutput = tx.outputStates.single() as KYCState
                "No inputs should be consumed" using (tx.inputs.isEmpty())
                "Only one output state should be created" using (tx.outputs.size == 1)
                "The output should be UserState" using (tx.getOutput(0) is KYCState)
                "The Signer must be signed" using (command.signers.toSet() == regOutput.parties.map { it.owningKey }.toSet())
            }

            is Commands.Update ->{

                val upInput = tx.inputStates.single() as KYCState
                val upOutput = tx.outputStates.single() as KYCState
            ...
            is Commands.Verify ->{
                ...
                val verInput = tx.inputStates.single() as KYCState
                val verOutput = tx.outputStates.single() as KYCState
                "Transaction must have one input" using (tx.inputs.size == 1)
                "Transaction must have one output" using (tx.outputs.size == 1)
